...
final class FileManager {
	...
	public function read( string $path, string $encoding = 'auto' ) {
		...
		if ( '' === $resolved || ! is_file( $resolved ) ) {
			return new \WP_Error( 'root_for_agents_not_found', sprintf( 'File not found: %s', $path ) );
		}
		if ( ! is_readable( $resolved ) ) {
			return new \WP_Error( 'root_for_agents_unreadable', sprintf( 'File not readable: %s', $path ) );
		}
		...
		if ( false === $bytes ) {
			return new \WP_Error( 'root_for_agents_read_failed', sprintf( 'Failed to read: %s', $path ) );
		}
	...
	public function write( string $path, string $contents, string $encoding = 'utf8', bool $append = false ) {
		$resolved = Helpers::resolve_path( $path );
		if ( '' === $resolved ) {
			return new \WP_Error( 'root_for_agents_bad_path', 'No path provided.' );
		}

		$existed = is_file( $resolved );
		$dir     = dirname( $resolved );
		if ( ! is_dir( $dir ) && ! wp_mkdir_p( $dir ) ) {
			return new \WP_Error( 'root_for_agents_mkdir_failed', sprintf( 'Could not create directory: %s', $dir ) );
		}

		$payload = 'base64' === $encoding ? base64_decode( $contents, true ) : $contents;
		if ( false === $payload ) {
			return new \WP_Error( 'root_for_agents_bad_base64', 'contents was not valid base64.' );
		}
		...
		if ( false === $written ) {
			return new \WP_Error( 'root_for_agents_write_failed', sprintf( 'Failed to write: %s', $path ) );
		}
	...
	public function delete( string $path, bool $recursive = false ) {
		...
		if ( '' === $resolved || ! file_exists( $resolved ) ) {
			return new \WP_Error( 'root_for_agents_not_found', sprintf( 'Path not found: %s', $path ) );
		}

		if ( is_dir( $resolved ) ) {
			$ok = $recursive ? $this->rmdir_recursive( $resolved ) : @rmdir( $resolved ); // phpcs:ignore WordPress.PHP.NoSilencedErrors
			if ( ! $ok ) {
				return new \WP_Error( 'root_for_agents_delete_failed', sprintf( 'Failed to delete directory (use recursive for non-empty): %s', $path ) );
			}
		} elseif ( ! @unlink( $resolved ) ) { // phpcs:ignore WordPress.PHP.NoSilencedErrors
				return new \WP_Error( 'root_for_agents_delete_failed', sprintf( 'Failed to delete: %s', $path ) );
		}
	...
	public function list( string $path, bool $recursive = false, int $max_depth = 5 ) {
		...
		if ( '' === $resolved || ! is_dir( $resolved ) ) {
			return new \WP_Error( 'root_for_agents_not_dir', sprintf( 'Not a directory: %s', $path ) );
		}
