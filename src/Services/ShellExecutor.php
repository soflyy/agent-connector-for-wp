...
final class ShellExecutor {
	...
	public function run( string $command, ?string $cwd = null, ?int $timeout_ms = null ) {
		if ( ! function_exists( 'proc_open' ) ) {
			return new \WP_Error( 'root_for_agents_proc_open_unavailable', 'proc_open() is disabled on this server.' );
		}
		if ( '' === trim( $command ) ) {
			return new \WP_Error( 'root_for_agents_empty_command', 'No command provided.' );
		}
		...
		if ( ! is_resource( $process ) ) {
			return new \WP_Error( 'root_for_agents_proc_open_failed', 'Failed to start the process.' );
		}
