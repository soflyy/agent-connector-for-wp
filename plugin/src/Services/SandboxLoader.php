...
final class SandboxLoader {
      ...
      public function detect_crash_on_shutdown(): void {
              ...
              if ( $is_fatal_error ) {
                      ...
                      $data = array(
                              'message' => 'Sandbox file ended the request early (exit/die) instead of returning contr
                      );
              }
      ...
      public function render_safe_mode_notice(): void {
              ...
              if ( $crashed ) {
                      $clear_button = sprintf(
                              '<form method="post" action="%1$s" style="display:inline;margin-left:.5em;"><input type=value="%2$s" />%3$s%4$s</form>',
              ...
              printf(
                      ...
                      esc_html__( 'A sandbox PHP file caused a fatal error, so all sandbox files are temporarily disabup.', 'agent-connector-for-wp' ),
                      wp_kses( $details, array( 'code' => array() ) ),
                      esc_html__( 'To recover: fix or delete the broken file in the sandbox directory, then clear safe".crashed" marker (or use the button).', 'agent-connector-for-wp' ),
                      wp_kses(
                              $clear_button,
                              array(
                                      'form'   => array(
      ...
      public function handle_clear_safe_mode(): void {
              if ( ! current_user_can( Config::CAP ) ) {
                      wp_die( esc_html__( 'You do not have permission to do this.', 'agent-connector-for-wp' ), '', ar ) );
              }
