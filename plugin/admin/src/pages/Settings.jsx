...
function Toggle({ checked, onChange, disabled }) {
  return (
    <button
      ...
      className={[
        'relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:opacity-40 disabled:cursor-not-allowed',
...
function Row({ label, description, control, danger }) {
  return (
    <div className={`px-6 py-4 flex items-start justify-between gap-6 ${danger ? 'bg-red-50' : ''}`}>
      <div className="flex-1 min-w-0">
        <div className={`text-sm font-medium ${danger ? 'text-red-800' : 'text-gray-800'}`}>{label}</div>
        {description && (
          <p className={`text-xs mt-0.5 ${danger ? 'text-red-600' : 'text-gray-400'}`}>{description}</p>
        )}
      </div>
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-gray-500">Configure the MCP server and plugin behaviour.</p>
      </div>
